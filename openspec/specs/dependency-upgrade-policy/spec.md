...
Defines the policy for upgrading third-party dependencies across the monorepo: per-library atomic commits with full quality gates, three risk-tiered sequential PRs, codemod-first migrations, catalog vs `package.json` version source of truth, human smoke-test gating for auth/UI bumps, per-commit and per-PR rollback guarantees, pre-flight baseline verification, commit message format, and re-verification of latest versions at execution time.
