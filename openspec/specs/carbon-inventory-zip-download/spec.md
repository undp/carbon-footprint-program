# carbon-inventory-zip-download Specification

## Purpose

Defines the behavior of the carbon inventory "Descargar" action, which produces a single ZIP bundle containing the emissions summary workbook, the methodology workbook, a human-readable README, and every active line file. Covers the ZIP layout, the client-side streaming generation, the supporting files-manifest and user-scoped methodology-export endpoints, and the fail-whole behavior on any fetch failure.
