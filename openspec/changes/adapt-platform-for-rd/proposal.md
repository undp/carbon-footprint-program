...
- Solid-waste disposal: add open dump and controlled dump alongside the sanitary landfill, incineration and recycling already offered, plus `Otro (especifique)`.
...
**Schema** — one additive migration: `secondary_subsector_id` and `territory_id` on `organization_data`, plus a new self-referencing `territory` table and the ten planning regions, thirty-two provinces and 157 municipios that populate it. Both columns are nullable and the table starts empty, so the migration applies to a populated database without rewriting a row.
...
**External dependencies** — MMARN must validate the 66-activity mapping; the Dominican emission factors for the new dimension values need documented sources; the territorial catalog comes from article 7 of Ley 345-22.
