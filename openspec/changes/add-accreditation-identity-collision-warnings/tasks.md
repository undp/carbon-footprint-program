...
- [x] 2.8 Build one warning per colliding SNAPSHOT (per design D7): an org that matches on both its approved and pending snapshots yields two separate warnings (one `APPROVED`, one `PENDING`), not merged; within a state compare against the org's CURRENT identity only — newest approved snapshot, resolved independently of the field filter, so a superseded snapshot never produces a warning and fields are never unioned across snapshots; order `APPROVED` before `PENDING`
