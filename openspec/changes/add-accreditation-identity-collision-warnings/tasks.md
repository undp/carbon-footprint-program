...
- [x] 2.3 Create the feature `apps/api/src/features/submissions/getSubmissionWarnings/` with `route.ts` (`GET /submissions/:id/warnings` under the admin router, ADMIN/SUPERADMIN), `handler.ts`, `service.ts`, `helpers.ts`
...
- [x] 3.4 Render one collapsed chip per conflicting org (state · tax id · legal name); the state chip goes through `<StatusChip>` + a `COLLISION_STATE_CONFIG` in `apps/web/src/labels/chips/` (label + tooltip + `StatusFamily` colors, composed from `VOCAB`), per the status-chip convention
...
- [x] 4.1 Build the comparison sub-component: on chip expand, show a side-by-side table of applicant vs conflicting org over `tradeName`, `legalName`, `taxId`
