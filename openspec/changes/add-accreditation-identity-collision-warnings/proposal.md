...
- **Generic `Warning` shape** — an intentionally generic bag `{ type, message, metadata }` so future submission types can add their own warning kinds without changing the contract.
- **Web — review dialog:** new "Conflictos detectados" section in `ViewSubmissionDialog`, shown only for organization-accreditation submissions that have warnings. Collapsed chips grouped by state (accredited first, then pending); each chip expands to a side-by-side comparison of the applicant vs. the conflicting org over the three identity fields, highlighting the colliding cell.
...
- **UI language** — all new user-facing strings in Spanish (no i18n), consistent with the app. Vocabulary follows `VOCAB` ("organización", "inscrita"), including the server-built warning message.
