...
A "Conflictos detectados" amber section between `CurrentStatusBanner` and `OrgDataSection`, whose subtitle says outright that the information is referential and the request can be approved anyway — the section informs a decision, it does not gate it.

That subtitle only holds **while there is a decision to inform**. Only a `PENDING` submission can be approved, rejected or returned (`updatePendingSubmissionStatus` filters on `PENDING`), so offering approval on an already-approved — or rejected, or returned — postulation states something the UI cannot do. The section still renders for those, because the conflicts a postulation was approved against are worth reading afterwards; the subtitle switches to presenting them as a record. It does not name the status: `CurrentStatusBanner` sits directly above and already carries it, so naming it again would be the same fact in two places, drifting the day a label changes. Collapsed: one numbered row per conflicting org ("Conflicto N" only), flat and in the endpoint's order. Expanded: a side-by-side grid (applicant vs that org) with the two status rows first and the three identity rows below, the colliding cells highlighted. **Alternatives considered:** a minimal chips + `OrgDataSection` re-use (less guided comparison), and an always-visible full matrix (clearest but heavy for the common single-conflict case). Hybrid is compact by default and rich on demand, and scales from 1 to N conflicts.
...
### D7 — Every in-funnel state per organization, each against its current snapshot

Detection matches the applicant against each other org's approved snapshot (if accredited), pending snapshot (if it has a pending submission), and returned-with-observations snapshot while that round is still open. If the **same** organization matches in more than one state, emit one warning per state — `collisionState = APPROVED` / `PENDING` / `REVIEWED` — rather than collapsing them. Warnings are ordered `APPROVED`, `PENDING`, `REVIEWED`: the registry first, then what is under adjudication, then what is waiting on the organization. Rationale: keep the full picture visible — the org is both officially registered with the colliding value and has a pending edit that also collides. (Resolves former open question on dedup.)

`REVIEWED` is a candidate state because a returned request has **not left the funnel**: the organization is expected to correct it and re-submit, so an admin approving the applicant is deciding against an identity that is about to come back. Nothing about that snapshot is approvable today, which is exactly why the copy names it "la postulación con observaciones" and never implies adjudication. `REJECTED` is _not_ a candidate: it is out of the funnel and its data is marked OUTDATED on the organization's next request. That exclusion used to hold only by omission (the branches enumerate states, so anything unlisted drops out); it is now pinned by a test, because the omission would break silently the day a branch is widened.

**Each state contributes at most one warning per organization, built from that organization's CURRENT identity in that state.** The states are not symmetric:

- **PENDING** — at most one per organization, enforced: `updateOrganization` refuses a second edit while one is under review (`OrganizationUnderReviewError`), org-wide and inside the transaction.
- **APPROVED** — an organization _accumulates_ approved snapshots, because approving never marks the previous one OUTDATED (`OUTDATED` is only used for rejected data). Only the **newest** is its identity: `organization_summary_view` resolves the displayed row the same way (`ORDER BY <status priority>, od.id DESC`). The older ones are history.
- **REVIEWED** — an organization also accumulates returned snapshots (re-submitting **clones** the returned row: `requestOrganizationAccreditation` → `cloneOrganizationData`), so the newest one is taken, and only while the round is **open**.

"Open" is defined as _still the organization's latest snapshot_ among those reachable through an approved, pending or returned submission — not as "has no approved snapshot". Both readings agree on the common histories, and only the id-based one gets the two edge cases right:

- returned → re-submitted (or → approved): the newer snapshot is a clone carrying the **same** identity, so reporting the returned row too would announce one collision twice under two states. The newer state wins.
- inscribed org whose **edit** was returned: its approved snapshot is _older_, so the returned edit is still the latest and stays visible. A "has no approved snapshot" rule would have hidden it.

A consequence worth stating: `PENDING` and `REVIEWED` are mutually exclusive for an organization whose requests are serialized, so the practical maximum stays **two** warnings per organization — its registered identity plus whatever its latest in-funnel snapshot is.

Selecting the current snapshot must happen **independently of the field filter**. Filtering first and taking the newest of what matched still yields a superseded row: if an org's current approved `taxId` is 222 and an older approved snapshot carried 111, an applicant with 111 matches only the older row. So the query finds candidate _organizations_ by match, then re-reads their current approved identity — a re-read that can legitimately find no collision at all, which is the correct answer. No false negatives: an org whose current snapshot collides necessarily has a matching snapshot, so it is in the candidate set.

This makes two failure modes structurally impossible rather than handled.

**Alternative rejected — union the colliding fields across an org's snapshots and show the newest tuple.** It reports a field as matching while displaying a different value for it: with v1 `{Foo, 111}` and v2 `{Foo, 222}` both ACTIVE+APPROVED and an applicant `{Foo, 111}`, `taxId` entered the union through v1 while the tuple came from v2, so the grid highlighted a "match" reading 111 against 222.

**Alternative rejected — report every non-dominated snapshot separately.** Each warning is then internally consistent, but the older ones are collisions against identities the organization no longer holds, announced as "la organización inscrita (RUT 111)" when the registry says 222. Consistent and irrelevant. Historical identity reuse, if it ever matters, is a separate and separately-labelled signal — not a collision against the current registry.
...
- `{postulación}`: `la postulación aprobada` (`APPROVED`) / `la postulación pendiente` (`PENDING`) / `la postulación con observaciones` (`REVIEWED`) — an exhaustive record over the state, not a ternary, so a new state cannot silently borrow another's wording; the wording tracks the submission status chips ("Pendiente", "Con Observaciones") that the comparison grid renders for the same row.
...
- **An org may appear in two warnings** — its approved identity plus its latest in-funnel snapshot (`PENDING` or `REVIEWED`, never both while its requests are serialized). Intended per **D7**. Within a state it appears at most once, against its current snapshot. → Revisit dedup if it proves noisy in practice.

## Open Questions

All previously open questions are resolved:

- **Dedup** (accredited + pending-editing same org) → **D7**: keep both warnings (one `APPROVED`, one `PENDING`); no collapse. Within a state only the org's current snapshot is compared, so there is nothing to merge. A returned round is the one case that IS collapsed, and only when it is closed — a re-submission or approval carries the same cloned identity, so the newer state represents it.
