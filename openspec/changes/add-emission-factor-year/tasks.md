...
- [x] 6.9 Reject a referenced factor whose status is `DELETED` (`details.reason = DELETED`), checked first. An id that matches no factor at all is reconciled like a damaged snapshot instead: factors are only ever soft-deleted, so such an id is forged rather than stale. Covered on create and on update.
