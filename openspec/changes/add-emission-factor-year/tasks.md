...
- [x] 6.3 Select `year`, `status`, `rateMeasurementUnitId` and the rate unit's `denominatorMeasurementUnit` in that lookup. The status is selected, not filtered on, so a deleted factor can be told apart from an id that never existed. The denominator is selected so the postponed `activity-unit-factor-mismatch` fix can add its check to the same query. Leave a TODO naming that fix.
- [x] 6.4 When a referenced factor's year differs from the footprint's — a footprint with no year included — reject the request with `INVALID_EMISSION_FACTOR_REFERENCE` (`422`, `details.reason = YEAR_MISMATCH`), on create and on update alike. Check every reference before the transaction opens, so a rejected request persists nothing, the valid lines that travelled with it included. See design Decision 7 for why this rejects rather than reconciles.
- [x] 6.5 Split the three shapes that arrive with a null `baseFactorId` instead of reading all of them as manual: nothing frozen yet (no applied factor value — a line being filled in, or a direct-total line) is persisted as before; a custom source is a manual factor and is left alone; a _catalogue_ source with no reference is a snapshot damaged before PR 647, or a forged payload, and is reconciled — saved without a snapshot and without a result — rather than rejected, since it is not a selection the user can redo. Reading it as manual is what would make it permanent — it round-trips as `baseFactorId: null`, so no later save looks at it again, while the non-null source keeps `fieldValidationService` from flagging the line. `createLineResult` takes the verdict so a direct total, typed rather than computed, is never dropped.
- [x] 6.7 Reject a referenced factor that does not belong to the line's subcategory (`details.reason = SUBCATEGORY_MISMATCH`). Nothing else ties `baseFactorId` to the line, so a crafted payload could otherwise freeze a factor of an unrelated subcategory onto it, and the lookup is already paying for the round trip. An update reads its line's subcategory from the validation query rather than a second one. A subcategory belongs to one category of one methodology version, so the version is covered by the same comparison.
...
- [x] 6.9 Reject a referenced factor whose status is `DELETED` (`details.reason = DELETED`), checked first. An id that matches no factor at all is rejected with `404` and the existing `EmissionFactorNotFoundError` — `404` when the row does not exist, `422` when it exists but cannot be used, as the endpoint already does for a subcategory — before any write. Both covered on create and on update.
- [x] 6.10 Map `INVALID_EMISSION_FACTOR_REFERENCE` in `getApiErrorMessage` to one message for the three reasons — «El catálogo de factores cambió mientras editabas. Recarga la página para ver los factores disponibles.» — and show the API's message on every capture save that can be refused: `useEmissionCaptureSubmit`, and the manual-mode toggle, which persists every line of the form rather than only the edited ones.
