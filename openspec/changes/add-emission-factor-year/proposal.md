...
- **Database**: one required column, one migration in three steps (add nullable, backfill to 2025, set `NOT NULL`). The existing partial unique index `emission_factor_unique_subcategory_dims_source` is recreated with `year` as a fifth column. Because the column is `NOT NULL`, Postgres' default `NULLS DISTINCT` behaviour is not in play for it and no `COALESCE` wrapper is needed. No column is added to `carbon_inventory_line_factor`: the clearing rule plus the sync validation make a catalogue line's year always equal to its footprint's, so it is derivable rather than frozen.
