...
- **Apply that same clearing to the existing data, in the migration.** Dating the catalogue does not retroactively fix the footprints that used it: an editable footprint of any year other than the catalogue's keeps a total computed from factors it is no longer offered. Submitted and verified footprints are left untouched as the record of what was declared.
- **Stop a line from losing its factor identity when it is edited.** `useEmissionCaptureData` currently reloads every line with `baseFactorId: null`, so saving any edit writes a snapshot with no `emissionFactorId`. That breaks both guarantees above through an ordinary edit, and it is already a live defect: the verifier's report derives the gas breakdown and the row identity from that relation, so an edited line loses its breakdown today.
...
**This does break the API contract**: creating an emission factor now requires a field that did not exist. And because the column is `NOT NULL` with no default, the previous API version cannot insert into it — migrations run before containers are replaced, so creating a factor from the maintainer fails until the new container is up. Admin-only, low frequency, visible and retryable; accepted rather than papered over with a column default, since a default would let a factor be created without stating its year.
...
- **Database**: one required column and one migration — add the column nullable, backfill to 2025, set `NOT NULL`, then clear the catalogue-factor snapshots and results of every editable footprint whose year differs from the catalogue's. The existing partial unique index `emission_factor_unique_subcategory_dims_source` is recreated with `year` as a fifth column; because the column is `NOT NULL`, Postgres' default `NULLS DISTINCT` behaviour is not in play and no `COALESCE` wrapper is needed. No column is added to `carbon_inventory_line_factor`: the clearing, the sync validation and the identity fix together keep a catalogue line on its footprint's year, so it is derived rather than frozen.
...
- The launch procedure for loading and verifying the 2026 catalogue. With the year mandatory, a footprint of the current year has no factors until that set exists; coordinating who loads it and how it is checked is left outside this change.
