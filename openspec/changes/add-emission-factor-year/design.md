...
The database cannot include magnitudes reached through `rate_measurement_unit` in a unique index. Therefore `emission_factor` gains required `numeratorMagnitudeId` and `denominatorMagnitudeId` columns, each referencing `Magnitude`. The server derives them from the selected rate unit through its numerator/denominator measurement units. The API never accepts these fields from the client and recomputes them on factor create/update.

Dimension slots not required by the subcategory are normalized to `null` **for the comparison only**, never on persistence. A value a maintainer entered in an optional slot is real catalog data and stays on the row; it simply does not earn the factor a separate identity. Normalizing on write would silently discard that value, and the production migration would have had to erase it from existing factors. The consequence is that the application's check is marginally stricter than the partial index, which compares the raw dimension columns: two factors differing only in an optional slot are rejected by the application even though the constraint would permit them. That asymmetry predates this change — the previous `checkDuplicateEmissionFactor` already keyed on required dimensions only — and it errs in the safe direction, rejecting a little more than the constraint rather than less.
...
3. Add nullable `year`, `numerator_magnitude_id` and `denominator_magnitude_id`; backfill the magnitude pair, then make both magnitude IDs required. Dimension slots are left exactly as stored: normalization applies to the uniqueness comparison, never to persistence.
