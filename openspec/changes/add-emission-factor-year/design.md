...
**The derived year is only as good as the weakest of the three mechanisms, so they must agree on one question**: is this line catalogue-backed. The migration, the clearing and the reconciliation each answered it separately at first, and two of them answered it with `emissionFactorId` alone — which reads a snapshot damaged before PR 647 (no reference, but a real catalogue source) as manual, and lets it keep a factor of another year permanently. All three now encode the same rule: a reference, or a source that is not one of `CUSTOM_FACTOR_SOURCES`. The migration states it in SQL because it is a snapshot of one moment; the two runtime paths read the constant.
