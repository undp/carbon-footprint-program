...
Manual-factor lines are untouched. A snapshot with no reference but a catalogue source — damaged before PR 647, or forged — is still reconciled, saved without a snapshot and without a result, and so is an id that matches no factor at all: neither is a selection the user made and can redo, and refusing them would make the subcategory unsavable. A direct total is never dropped.
