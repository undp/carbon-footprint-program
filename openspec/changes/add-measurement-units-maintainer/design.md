...
- **[Pre-existing seed gaps in non-reseeded deployments]** → The seed-time coverage check in `seedMeasurementUnits.ts` (decision #8) prevents new gaps on any environment that re-runs the seed during deploy, but it does not retroactively repair an already-deployed database that upgrades without re-seeding. Such an environment will keep any pre-existing missing-canonical-RMU gap until an admin re-saves the affected MU through the maintainer screen (which re-triggers the cascade). Mitigation: deployment runbooks SHALL re-run `seedMeasurementUnits.ts` on upgrade so the coverage check fires; the cascade rule guarantees correctness from the first admin write forward.
