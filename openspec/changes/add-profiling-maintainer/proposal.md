...
- **Delete semantics**: delete endpoints MUST throw `DataIntegrityError` (HTTP 500, as currently defined) when the row is referenced by `CountrySubsector`, `OrganizationMainActivity` (via `countrySectorId` / `countrySubsectorId`), `OrganizationData` (via `sectorId` / `subsectorId`), or `SubcategoryRecommendation` (via `sectorId` / `subsectorId`). Spanish `userMessage` flows through `getApiErrorMessage` (which prefers the server-supplied `userMessage` and falls back to a code-keyed Spanish string).
...
- **Validation**: `name` as `z.string().trim().min(1).max(255)` (trim first so whitespace-only is rejected). `description` as `z.string().trim().max(2000).nullable().optional()`; PATCH tri-state (`undefined` = no-op, `null` = clear, `""` → normalized to null at service). PATCH body refined to require ≥1 field (empty `{}` → 400). Zod messages in Spanish.
...
- **API**: 8 new feature directories under `apps/api/src/features/countrySectors/admin/` and `apps/api/src/features/countrySubsectors/admin/`; new route-mount files `apps/api/src/routes/api/admin/country-sectors/index.ts` and `apps/api/src/routes/api/admin/country-subsectors/index.ts` following the `routes/api/admin/organizations/index.ts` pattern (`requireAuth` onRequest + `requireRoles([SUPERADMIN, ADMIN])` preHandler); shared errors reused from `apps/api/src/errors/`.
- **Frontend**:
