...
- [ ] 5.0a **Backend `userMessage` contract — cross-cutting prerequisite.** Audit `apps/api/src/commonSchemas/errors.ts` (the `ApiErrorResponseSchema`) and confirm it carries an optional `userMessage: string` field; if missing, add it. Audit the Fastify error-handler plugin and verify it serializes the thrown error's `userMessage` (when present) into the response body alongside `code` and `message` for `DatabaseUniqueConstraintViolationError`, `DeleteBlockedByReferencesError`, `NoCountryFoundError`, `ResourceNotFoundError`, and any other error class thrown from the new admin services. If the handler does not currently propagate `userMessage`, extend it so it does, and add a unit test asserting the field round-trips. Without this, the FE `getApiErrorMessage` fallback will never see a Spanish `userMessage` from the API.
