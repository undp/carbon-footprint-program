...
- [ ] 5.5 `restoreCountrySector/`: wrap in `prisma.$transaction`. If the row's current `status` is already `ACTIVE`, throw a 400 error with Spanish text on `error.message`. Check for ACTIVE collision on `(countryId, name)` — if found, throw `DatabaseUniqueConstraintViolationError` (409). Otherwise update `status: ACTIVE`. Return the updated row.
