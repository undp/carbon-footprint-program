...
**The widget sends the OIDC access token.** Every other screen gets this from `apiClient`, whose `beforeRequest` hook attaches it; the chatbot cannot use that client (SSE needs the raw response body, and ky's `afterResponse` throws on the non-2xx bodies the widget renders as copy), so it attaches the header itself. Without it the API's permissive `requireAuth` — registered for the whole chatbot router in `routes/api/chatbot/index.ts` — resolves no user, `request.currentUser` stays unset, and `chatbotIdentityPreHandler` falls through to the anonymous branch. A signed-in person's conversations were therefore stored against the session cookie rather than their account, and disappeared with it; third-party cookie restrictions make that routine rather than rare. Anonymous visitors are unaffected: no header, and the session cookie remains their identity.
...
Note that sending the token makes that limitation _visible_ where it previously was not. Before, logging in changed nothing about the chatbot's identity, so a thread started anonymously simply continued. Now logging in genuinely switches identity, so that thread becomes unreachable at that moment. The trade was taken deliberately: the alternative — reassigning an anonymous row to the authenticated user on the first signed-in turn — is more code and more surface than V1 wants, and the degradation is graceful (404 → the widget drops its stored id → an empty thread, not an error).
