...
- [x] 3.5 Extend the `chatbot/no-network-imports-in-mock` ESLint rule scope in `apps/api/eslint.config.ts`: add a second `files: [...]` block applying the same rule to `apps/api/src/features/chatbot/embeddingProvider/mock.ts`. One rule, two file targets — do NOT duplicate the rule.
