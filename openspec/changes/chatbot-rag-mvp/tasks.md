...
- [x] 9.5 **[FOUNDATION FIX — PM DECISION]** "Nueva conversación" affordance behavior. The widget's "Nueva conversación" (`AddIcon`) click handler SHALL ONLY clear local React state — reset the message list to empty AND generate a fresh `conversation_id` (the next user message starts a new conversation thread for backend purposes). It SHALL NOT call any backend `DELETE` endpoint, and no such endpoint is added by this change. The persisted `chatbot_chat_conversation` and `chatbot_chat_message` rows in the database SHALL remain untouched — conversations are auditable, not user-deletable from the widget. Concretely:
  - Locate the existing "Nueva conversación" click handler in the widget (in `apps/web/src/components/Chatbot/` — header / toolbar component).
  - Confirm there is no `fetch`/`apiClient` call to a `DELETE /chat/conversations/...` (or similar) path; if found, remove it.
  - The handler SHALL call `setMessages([])` (or whatever local state mutation clears the message list) AND assign a `crypto.randomUUID()` (or existing UUID-generation helper) to the `conversation_id` state so the next outgoing `sendMessage` carries a fresh id.
  - Surface the button's tooltip/aria-label as `"Nueva conversación"` — NOT `"Limpiar conversación"` or `"Eliminar conversación"` (the wording communicates "start a fresh thread", not "delete the database").
  - Add a unit/integration test asserting that clicking the "Nueva conversación" button (a) clears the rendered message list, (b) does NOT trigger any HTTP request (assert via mocked `apiClient` / `fetch` spy that no DELETE is fired), and (c) the next outgoing `sendMessage` carries a `conversation_id` distinct from the one used before the click.
  - Rationale: PM-owned decision. Conversations are audit material; client-triggered hard delete is out of V1 widget scope. Documented in `design.md` Decision 25.
...
- [ ] 9.7 **DEFERRED to `chatbot-educate-mode-full`** per PM V1 scope decision. V1 does not expose a user-facing "Eliminar mi historial" affordance. The foundation-defined `DELETE /api/chatbot/conversations/me` endpoint exists and is operable by support staff for D11 right-to-be-forgotten requests in V1; the UI affordance (button + confirmation dialog + DELETE wiring from the widget) ships in V2/V3. V1 is intentionally not full-compliance-UI-complete; the regulatory obligation is met via support intake plus endpoint invocation. See `design.md` Decision 25 and `proposal.md` Deferred Debt entry.

- [x] 9.8 **[PM DECISION — persistence across reload]** Implement `GET /api/chatbot/conversations/me/current` and wire the widget to rehydrate on mount. Scope:
  - Backend: new feature folder `apps/api/src/features/chatbot/getCurrentConversation/{route,handler,service}.ts`. Reads the signed `chatbot_conversation_id` cookie; filters Prisma query by `id = cookieValue AND expires_at > NOW() AND <identity filter>` (the identity filter is `user_id = caller.userId AND session_id IS NULL` for authenticated callers, `session_id = caller.sessionId AND user_id IS NULL` for anonymous callers — strict, no anon→auth claim). 204 when cookie absent; 200 with `{ conversation, messages }` on hit; 404 with `Set-Cookie: chatbot_conversation_id=; Max-Age=0` on miss. Excludes `tokens_used`, `latency_ms`, `truncated` from the wire shape. Messages ordered ASC by `created_at`. Register the route in `apps/api/src/routes/api/chatbot/index.ts`.
  - Cookie helper: new `apps/api/src/features/chatbot/helpers/conversationCookie.ts` with read/write/clear helpers. Signed with `COOKIE_SECRET` (same secret as the session cookie). `httpOnly: false` (Decision 28); `Path=/api/chatbot`; `Max-Age = CHATBOT_CONVERSATION_TTL_DAYS * 86400`; `SameSite=Lax`; `Secure` only in production. The Set-Cookie write is multi-cookie-aware (reads existing `Set-Cookie` header and appends) so it does not clobber `chatbot_session_id` written earlier by the identity preHandler. The cookie is set inside the streaming handler immediately after `resolveOrCreateConversation` returns (sliding refresh on every successful turn), BEFORE `reply.hijack()` so `writeSseHeaders` forwards it.
  - Types: new `packages/types/src/chatbot/getCurrentConversation/{schemas,types}.ts` with the Zod response schema. Re-exported via `packages/types/src/chatbot/index.ts`.
  - Frontend: `useChatStream` issues `fetch(LOAD_URL, { credentials: "include" })` on mount; on 200 maps the response into `ChatbotMessage[]` and seeds `messages`; on 204 / 404 / network failure leaves the list empty. New `historyLoading` boolean prevents the empty-state placeholder from flashing before the seed lands. `startNewConversation` ALSO drops the cookie client-side via `document.cookie = "chatbot_conversation_id=; path=/api/chatbot; max-age=0; SameSite=Lax"` so a subsequent reload does not re-hydrate the prior thread.
  - Tests: new `apps/api/test/features/chatbot/getCurrentConversation/integration.test.ts` covers 204 no-cookie, 200 happy path with ordering and sources_cited preservation, K=0 (empty array passthrough), 404 expired with cookie-clear header, 404 cross-user (IDOR guard with a freshly-created second user row), 404 anon-conv + auth-req (V1 anon→auth NOT supported per Decision 28), 404 tampered/unsigned cookie, and the internal observability columns (`tokensUsed` / `latencyMs` / `truncated`) absent from the wire. Tests for the anonymous-request path are not added at the HTTP layer because the vitest setup pins `AUTH_PROVIDER=forced-user` and there is no per-test mechanism to disable it — the cross-user and anon-conv-with-auth-req tests cover the strict identity match invariant from the IDOR angle that matters.

- [x] 9.9 Update `openspec/changes/chatbot-rag-mvp/specs/chatbot-widget/spec.md` — add ADDED requirement "Widget restores active conversation on mount via signed conversation cookie" with the five scenarios (cookie absent → 204, cookie valid → 200 hydrate in order, cookie expired → 404 + clear, cookie cross-identity → 404 + clear, Nueva-then-reload → empty). Modify the existing "Nueva conversación" requirement to add the cookie-clear step (point 3) and a new scenario asserting the cookie is dropped from `document.cookie` after the click.

- [x] 9.10 Update `openspec/changes/chatbot-rag-mvp/specs/chatbot-message-streaming/spec.md` — add ADDED requirement "Streaming handler sets a signed chatbot_conversation_id cookie on every turn that resolves or creates a conversation" with three scenarios (first-turn create, sliding refresh on subsequent turns, does-not-clobber-session-cookie).
...
- [x] 10.5 `ingest/integration.test.ts > re-ingest with new version creates new DRAFT alongside existing ACTIVE` — seed an ACTIVE v05; invoke ingest with `--version v06`; assert two rows exist (v05 ACTIVE, v06 DRAFT).
- [x] 10.6 `ingest/integration.test.ts > re-ingest collision on existing DRAFT fails fast with non-zero exit and Spanish error message` — seed a v05 DRAFT; invoke ingest with `--version v05`; assert non-zero exit code and stderr Spanish message naming the colliding source's id; assert no new rows inserted.
- [x] 10.7 `ingest/integration.test.ts > audit row persists on embedding failure with NULL completed_at` — set `EMBEDDING_PROVIDER=mock` but stub the mock to throw; invoke ingest; assert no `chatbot_corpus_source` row exists for the attempt; assert one `chatbot_corpus_ingest_run` row exists with NULL `completed_at` and `source_id`.
- [x] 10.8 `activate/integration.test.ts > flips OUTDATED on previous + ACTIVE on new in single transaction` — seed an ACTIVE v05 and a DRAFT v06 of the same `(name, scope)`; invoke activate on v06's id; assert v05 transitions to OUTDATED with non-NULL `deactivated_at`, v06 transitions to ACTIVE with non-NULL `activated_at`, both updates share commit timestamp.
- [x] 10.9 `activate/integration.test.ts > advisory lock serializes concurrent activates with same (name, scope)` — seed two DRAFTs of the same `(name, scope)`; fire two concurrent `chatbot:activate` invocations targeting them via `Promise.allSettled`; assert exactly one ACTIVE row remains for that `(name, scope)` after both settle.
- [x] 10.10 `activate/integration.test.ts > refuses non-DRAFT target with explicit Spanish error` — seed an OUTDATED row; invoke activate on its id; assert non-zero exit code, Spanish error message naming the current status, no row modified.
- [x] 10.11 `searchKnowledge/integration.test.ts > scope and sourceType filters restrict results` — seed mixed ACTIVE sources of different `scope` and `source_type`; query with each filter combination; assert results match.
- [x] 10.12 `searchKnowledge/integration.test.ts > empty query throws InvalidQueryError` — invoke `searchKnowledge("")`, `searchKnowledge("   ")`; assert each throws an error whose `name === "InvalidQueryError"`.
- [x] 10.13 `searchKnowledge/integration.test.ts > out-of-range topK throws InvalidQueryError` — invoke with `topK = 0` and `topK = 21`; assert each throws `InvalidQueryError`.
...
- [x] 10.15 `embeddingProvider/unit.test.ts > mock-in-prod boot guard throws` — set `NODE_ENV=production` and `EMBEDDING_PROVIDER=mock`; import `apps/api/src/config/environment.ts`; assert the import throws.
- [x] 10.16 `embeddingProvider/unit.test.ts > getEmbeddingProvider returns mock for EMBEDDING_PROVIDER=mock and Azure for azure-openai; throws on banana` — three subcases.
...
- [x] 10.22 `ChatbotWidget/MessageBubble tests > MessageBubble renders Fuentes consultadas panel when sourcesCited non-empty` — render a `<MessageBubble>` with an assistant message carrying `sourcesCited` of length 2; assert a `Collapse`-driven panel exists with header `"Fuentes consultadas (2)"`; assert each row has an anchor with `target="_blank"` and `rel="noopener noreferrer"`.
- [x] 10.23 `ChatbotWidget/MessageBubble tests > MessageBubble does not render panel when sourcesCited absent or empty` — two sub-cases.
...
- [x] 10.26 `embeddingProvider/unit.test.ts > Azure batcher splits on 17+ inputs even when token sum is small` — invoke the azureOpenAI provider's batcher with 17 short inputs whose cumulative `estimateTokens` is well below 8192; assert ≥ 2 SDK calls were issued (use a stub SDK to count invocations); assert no SDK call's `input` array exceeded length 16.
- [x] 10.27 `embeddingProvider/unit.test.ts > Azure batcher splits on token cap even with few inputs` — invoke the batcher with inputs whose cumulative `estimateTokens` exceeds 8192 in fewer than 16 strings; assert ≥ 2 SDK calls and no call exceeded the 8192-token sum.
- [x] 10.28 `ChatbotWidget/MessageBubble tests > "Nueva conversación" click clears local state only` — render the widget with a populated message list; spy on `apiClient`/`fetch`; click the "Nueva conversación" button (`AddIcon` in the widget header); assert (a) the rendered message list is empty, (b) zero HTTP requests were issued by the spy, (c) the button's accessible name is `"Nueva conversación"`. The original clause about the client minting a fresh `conversation_id` no longer applies: the server owns the conversation id and pins it with the signed `chatbot_conversation_id` cookie, which the reset drops — asserted in `useChatStream.test.ts`. Foundation regression guard for Task 9.5.
...
- [x] 10.36 `toolRound/integration.test.ts > Modo C conversational, welcome, and off-domain behaviors` — split into three sub-suites:
  ...
  **(c) Off-domain redirect (Sub-modo C.1)** — POST `"¿cuál es la población de Marte?"`, `"¿cuánto es 2+2?"`, and `"¿cómo está el clima en Santiago?"` as three sub-cases. For each, assert ALL of: (i) zero `tool_call` events AND a server-side spy on `executeSearchKnowledgeTool` confirms it was NOT invoked; (ii) the joined assistant content equals byte-for-byte exactly the off-domain redirect literal `"Solo puedo ayudarte con preguntas sobre metodología de huella de carbono, factores de emisión, los alcances 1, 2 y 3, y el uso de la plataforma Huella Latam. ¿En qué de esos temas te puedo ayudar?"`; (iii) the assistant row's `sources_cited` is `[]`; (iv) the `done` event payload does NOT include a `sources` field. Aligns the bot's V1 Educar behavior with the documented "Lo que el bot NO es" exclusion of generalist-assistant responses on factual off-domain questions.
...
- [ ] 10.38 **DEFERRED with the feature it tests** — covers the "Eliminar mi historial" affordance and its confirmation/DELETE flow, which is deferred to `chatbot-educate-mode-full` per PM V1 scope decision (see Task 9.7). When the affordance ships in V2/V3, this test moves alongside it. Compliance regression guard for D11 remains the support-operated `DELETE /api/chatbot/conversations/me` endpoint, which already has API-side integration test coverage from foundation's `chatbot-conversation-deletion` capability — not a widget test.
...
- [x] 12.2 Run `pnpm test --filter=api -- /chatbot --coverage=false`. All chatbot integration and unit tests SHALL pass, including the four CRITICAL tests (10.1, 10.2, 10.3, 10.4).
  - **Note:** 12.2 verified for chatbot scope (63/63 passing). Full API suite has 5 pre-merge known failures in non-chatbot features (`carbonInventories/getCarbonInventoryMethodology`, `countrySectors/getAllCountrySectors`, `jobPositions/getAllJobPositions`, `measurementUnits/getAllRateMeasurementUnits`, `organizationMainActivities/getAllOrganizationMainActivities`) caused by collation divergence between alpine/musl and debian/glibc after the pgvector image bump (task 1.1). Pending team coordination — see operator note.
- [ ] 12.3 Run `pnpm test --filter=web -- chatbot --coverage=false` (or the equivalent for the widget tests). The MessageBubble citation-panel tests SHALL pass.
  - **Implemented** in `chatbot-web-test-infra` (see that change): the web test toolchain landed on `main` independently (#496 / #513), so 10.22, 10.23, 10.28 and 10.34 are covered here by `MessageBubble.test.tsx`, the new `ChatbotWidget.test.tsx`, and `useChatStream.test.ts`. Task 10.38 remains deferred together with the affordance it tests (see Task 9.7) and is NOT part of `chatbot-web-test-infra`.
- [x] 12.4 Local end-to-end smoke matching the Criterio de éxito stated in `proposal.md`: with `LLM_PROVIDER=azure-openai`, `EMBEDDING_PROVIDER=azure-openai`, `AZURE_OPENAI_API_KEY` set, and a manually-provisioned Azure OpenAI resource:
  - **Note:** verified against real Azure OpenAI with a 5-page GHG Protocol Corporate Standard fair-use excerpt. All four scenarios passed end-to-end: Modo A covered-case (inline citations plus "Fuentes consultadas" panel), Modo A K=0 fallback (opener literal, no invented links), Modo B platform redirect (literal response, no tool call), Modo C welcome (capability disclosure within 2-6 sentences).
