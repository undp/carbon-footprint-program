...
- [ ] 10.28 `widget/render.test.ts > "Nueva conversación" click clears local state only` — render the widget with a populated message list; spy on `apiClient`/`fetch`; click the "Nueva conversación" button (`AddIcon` in the widget header); assert (a) the rendered message list is empty, (b) zero HTTP requests were issued by the spy, (c) sending a new message after the click carries a `conversation_id` distinct from the one before, (d) the button's aria-label is `"Nueva conversación"`. Foundation regression guard for Task 9.5.
...
- [ ] 12.3 Run `pnpm test --filter=web -- chatbot --coverage=false` (or the equivalent for the widget tests). The MessageBubble citation-panel tests SHALL pass.
  - **Deferred** to `chatbot-web-test-infra` follow-up change: `apps/web` has no test runner in V1 (`"test": "echo 'Not implemented yet'"`). The widget tests listed in section 10 (10.14, 10.22, 10.23, 10.28, 10.34) are formalized in `openspec/changes/chatbot-web-test-infra/` and will be implemented alongside the web test infrastructure setup. Task 10.38 is deferred together with the affordance it tests (see Task 9.7) and is NOT part of `chatbot-web-test-infra`.
