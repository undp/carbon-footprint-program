...
The CLI SHALL insert one row into `chatbot_corpus_ingest_run` for every invocation that passes argument validation. Argument-parsing failures (missing `--cite-url`, invalid `--source-type`, label containing `:`, etc.) exit non-zero BEFORE any database write per the argument-validation requirements above and SHALL NOT insert an audit row — the precedence is: parse args → if valid, insert audit row → embedding-and-source transaction. Once inserted, the audit row carries `started_at = NOW()`, `triggered_by` populated, `source_id = NULL`, `completed_at = NULL`. On success, the CLI updates it with `source_id = <new id>`, `completed_at = NOW()`, `chunks_created = N`, `embedding_model = <resolved model>`. On failure of any post-validation step (parsing/chunking, embedding, source/chunks transaction) it remains in the initial state — providing a visible trace of the failed attempt.
...
The system SHALL provide `apps/api/scripts/chatbot/activateCorpusSource.ts`, registered as `"chatbot:activate"`, invoked as `pnpm --filter api chatbot:activate <source-id>`. The script runs a single Prisma interactive transaction. The transaction SHALL: (1) load the target row's identity (`name`, `scope`) by `id` so the lock key can be computed; (2) acquire an identity-scoped advisory lock via `SELECT pg_advisory_xact_lock(('x' || substr(md5($key), 1, 16))::bit(64)::bigint)` with `$key = 'chatbot-corpus:' || name || ':' || scope` BEFORE any DRAFT validation or state changes; (3) re-read the target row under the held lock to defend against a concurrent activate that flipped the row between steps (1) and (2). The md5-cast-to-bigint form is used (instead of `hashtextextended($key, 0)`) for portability across all Azure Postgres Flexible Server image variants targeted by this codebase — `hashtextextended` is not guaranteed to be exposed at the SQL surface across every minor version and extension set. `hashtext($key)` (32-bit) is an acceptable simpler alternative; the 64-bit md5 form is preferred because it matches the advisory-lock space width and reduces cross-key collision risk to negligible levels for the small population of `(name, scope)` tuples expected at V1 scale (cryptographic hashes still have non-zero collision probability — the claim is "negligible", not "impossible"). Under the held lock: (a) verify target exists with `status = 'DRAFT'`, (b) flip prior `ACTIVE` of the same `(name, scope)` (if any) to `OUTDATED` with `deactivated_at = <now>`, (c) set target to `ACTIVE` with `activated_at = <now>` — both columns SHALL share the same `<now>` instant captured once per transaction, so the cutover is atomic at the timestamp level too. The transaction commits atomically; the lock releases on commit/rollback.
