...
- **Database** (`packages/database/src/prisma/schema.prisma`): the `ReductionProject` deferred columns become nullable (`implementationDate`, `description`, `subcategoryId`, `year`, `baselineScenario`, `projectScenario`; `subcategory` relation optional). Per repo dev-phase convention, the existing create-table migration is **edited in place** (no incremental migration), then the Prisma client is regenerated.
...
- **Delivery**: large full-stack change shipped as **chained PRs** — PR1 = DB + types + utils + API + integration tests; PR2 = web.
