...
- Introduce a `StorageAdapter` facade (`apps/api/src/services/storage/`) that defines a backend-agnostic contract for read/write presigned URLs, head/delete/copy/move, and health check.
...
- Add MinIO + `minio-init` (bucket bootstrap via `mc`) services to `docker-compose.yml`, with `.envrc.template` entries for the new variables.
...
- `object-storage`: Backend-agnostic object storage capability (presigned URL issuance, head/delete/copy/move, health checks) selected at runtime by `STORAGE_PROVIDER`. Defines the `StorageAdapter` contract, the `HttpUploadMethod` enum exchanged with clients, and the upload-response shape returned by `requestUpload`-style endpoints.
...
- **Infra**: `docker-compose.yml` gains `minio` + `minio-init`. `.envrc.template` gains `STORAGE_PROVIDER` and `MINIO_*` vars. MinIO requires a CORS allow-origin for browser presigned PUTs; documented in `FileStorage.md`.
...
- **Tests**: `apps/api/test/setup/testcontainers.ts` branches on provider; `appFactory.ts` constructs the adapter via `createStorageAdapter(provider, env)` instead of `BlobServiceClient.fromConnectionString` directly; `blobHelper.ts` → `storageHelper.ts` with `uploadFixture` that uses `generateWriteUrl` + `fetch` (works against either backend).
