...
- Add an `LLMProvider` interface with two implementations: `mock` (deterministic eco template) and `azureOpenAI` (managed-identity, written but not exercised in tests). Selection via `LLM_PROVIDER` env var; `mock` rejected at boot when `NODE_ENV=production` **and** the chatbot is enabled (see the `CHATBOT_ENABLED` bullet below — a disabled deployment needs no provider at all).
...
- Gate the entire chatbot behind a single `CHATBOT_ENABLED` env flag, **default off** (opt-in). When disabled: the `routes/api/chatbot` group registers nothing (endpoints 404), the `LLM_PROVIDER` / Azure / `COOKIE_SECRET` boot guards are skipped, and the widget is not mounted (mirrored on the frontend by `VITE_CHATBOT_ENABLED`). A default deployment therefore runs the full platform with no AI code path and no cloud dependency — the DPG optionality principle (see `design.md` §Decisions, 8).
- Add `<ChatbotWidget />` mounted at the web root (`apps/web/src/routes/__root.tsx`, only when `VITE_CHATBOT_ENABLED`) so it is reachable from both the public landing and the authenticated app. Streams via raw `fetch` + `ReadableStream` (the existing `ky` client does not support SSE). Because `POST /api/chatbot/message` is non-idempotent, the widget does **not** auto-retry: each turn is a single attempt guarded by a per-turn `AbortController` (idle + overall client timeout, a user **Stop** control, and abort-on-unmount). Failures escalate via a consecutive-failure counter — one failed turn is a recoverable `error` (input stays enabled), and two consecutive transport-level or `5xx` failures flip the widget to a **non-blocking** `degraded` state that surfaces an honest "assistant unavailable" message while keeping the input enabled (no polling, no background requests; a functional non-streaming fallback is deferred to V1, per `design.md` §Decisions, 7). Mid-turn disconnects always result in `truncated = true` with no replay — the widget surfaces the truncation to the user and the next turn is a fresh request.
...
- `chatbot-widget`: `<ChatbotWidget />` minimum-viable React component. Streams via raw `fetch` + `ReadableStream`; does not auto-retry (each turn is a single `AbortController`-guarded attempt) and enters a **non-blocking** `degraded` state after two consecutive transport-level or `5xx` failures, keeping the input enabled (a functional non-streaming fallback is deferred to V1). Final visual treatment, copy, ARIA streaming announcements, mobile responsiveness, and discoverability are explicitly deferred to a separate design review.
...
- **Frontend**: New components folder `apps/web/src/components/Chatbot/` containing `ChatbotWidget.tsx` and `useChatStream.ts`. Widget mounted in `apps/web/src/routes/__root.tsx` (only when `VITE_CHATBOT_ENABLED`) as a minimum-viable placement (subject to design review). No new web dependencies — reuses existing `react-markdown` for rendering markdown in messages.
...
- **Docs**: New chatbot retention/persistence section in `docs/security/sensitive-data.md`. New `CHATBOT_ENABLED`, `LLM_PROVIDER`, and `COOKIE_SECRET` entries in `docs/development/environment-variables.md`. Note in `docs/operations/runbook.md` declaring the pg_cron purge job and the Bicep dependencies as a separate infra change.
...
- **CORS for cross-site widget**: in production `apps/web` and `apps/api` are served from different registrable domains (cross-site), so the `chatbot_session_id` cookie is set `SameSite=None; Secure` to ride the frontend's `credentials: "include"` requests; in local dev it is `SameSite=Lax` (the Vite proxy keeps the widget same-origin over plain HTTP). The existing `cors-plugin` already reflects `ALLOWED_ORIGIN` with `credentials: true`, which supports the cross-site credentialed case — no code-side CORS change is required in this foundation.
