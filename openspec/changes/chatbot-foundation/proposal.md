...
- Add `DELETE /api/chatbot/conversations/me` — idempotently deletes all conversations bound to caller identity. Anonymous callers identified by signed `session_id` cookie; authenticated callers by `user_id`.
...
- Add a feature-local preHandler under `apps/api/src/features/chatbot/` that resolves caller identity (`request.currentUser` if authenticated, otherwise `session_id` cookie) without 401-ing on missing auth.
...
- `chatbot-conversation-persistence`: Schema and identity-scoping rules for `chatbot_chat_conversation` and `chatbot_chat_message`. Defines how anonymous (cookie `session_id`) and authenticated (`user_id`) callers map onto conversation rows, the 30-day `expires_at` semantics, and the index plan that pre-positions for future Postgres-backed rate limiting.
...
- **Conventions**: Future chatbot-related OpenSpec changes use the `chatbot-` prefix in flat kebab-case (`chatbot-rag-ingest`, `chatbot-educate-mode`, etc.). Covers D10 of the planning document. The prefix is naming-only because OpenSpec 1.3.1 does not recurse into nested change subdirectories.
...
- **Privacy notice / consent UI** for V5 anonymous-to-authenticated data access (D12 of the planning document).
...
- **Symmetric dormant-insert scenarios in `chatbot-corpus-schema`** for `chatbot_corpus_chunk` and `chatbot_corpus_ingest_run` (only `chatbot_corpus_source` has one today). Adds two scenarios; pure documentation polish.
- **Tighten the SSE `id:` field semantics** in `chatbot-message-streaming` — the current spec says events carry "stable IDs" and the widget uses them for `Last-Event-ID`, but does not pin the source of those IDs (currently the assistant message row's `id` is the natural choice). Worth defining before a future server-side replay buffer is introduced.
...
- **Remove the residual scenario duplication** in `chatbot-conversation-deletion` where the tampered-cookie case is asserted under both "Caller without identity returns 204" and "Anonymous deletion clears the session cookie". The scenarios are compatible but the lector reads the same input twice. Refactor for clarity.
