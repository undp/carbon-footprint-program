...
- **BREAKING (internal contract)**: extend `UpdateUserBodySchema` into a tagged union (in the TypeScript sense) of `SelfProfileUpdate` and `AdminRoleUpdate`, implemented as a plain `z.union([SelfProfileUpdateSchema, AdminRoleUpdateSchema])` in Zod — **not** `z.discriminatedUnion(...)`, since neither branch carries an explicit discriminator literal field. Disjointness between branches is enforced by `.strict()` on each `z.object` plus `.refine` for the "at least one defined field" rule on the self-profile branch, so Zod differentiates the branches by field presence. The `SelfProfileUpdate` branch accepts any subset of the existing profile fields — `email`, `countryJobPositionId`, `firstName`, `lastName`, `idpUserId`, `idpName`, `termsAccepted` — and disallows `role`; the `AdminRoleUpdate` branch contains exactly `{ role: SystemRole }` and no profile fields. `idpName` stays in the schema and the `SelfProfileUpdate` branch because the service currently writes it (`updateUser/service.ts`); implementers must keep schema and service aligned on this field. Service-side branching in `updateUser/service.ts` keys off `"role" in body` (or equivalent shape detection) to dispatch — consistent with the union choice. Existing callers using the old shape continue to work because their bodies match the `SelfProfileUpdate` branch.
...
- Add error classes `SelfRoleChangeError` (403), `LastSuperadminError` (409), `InsufficientPermissionsError` (403), `InvalidRoleTransitionError` (409). Update the `PATCH /users/:id` route contract in `apps/api/src/features/users/updateUser/route.ts` to extend the `response` map with `403: ApiErrorResponseSchema` and `409: ApiErrorResponseSchema` (alongside the existing `200`/`400`/`404`/`422`) so the typed schema, Swagger docs, and runtime serialization match the new error shapes. Wire the same error codes into `getApiErrorMessage` in `apps/web/src/utils/getApiErrorMessage.ts` with Spanish copy so client-side runtime responses translate consistently.
