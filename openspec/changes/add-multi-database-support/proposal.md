...
- Introduce a **dual-provider data layer**: two `schema.prisma` files (`prisma/postgresql/`, `prisma/sqlserver/`) sharing the same logical model. The SQL Server schema is mechanically derived from the PostgreSQL one (`scripts/_derive_sqlserver_schema.py`). Both generate the active provider's client into a single shared `src/generated/prisma` directory, so consumer imports never branch on provider.
- **Enums**: SQL Server has no native enums, so a hand-authored `src/enums.ts` facade (re-exported from `@repo/database`) is the canonical source for both providers; PostgreSQL keeps native enums, SQL Server uses `String` + CHECK constraints.
...
- **JSON handling** (Prisma's SQL Server connector does NOT support the `Json` type):
  - Drop `@db.JsonB` on the 4 JSON columns and convert the 2 `String[]` columns (`SystemParameter.options`, `ReductionProject.consideredGei`) to `Json` — on PostgreSQL.
  - On SQL Server those 6 columns are `String @db.NVarChar(Max)`; a Prisma Client extension (`applySqlServerCompat`, no-op on PG) transparently `JSON.stringify`/`JSON.parse`s them so app/seed/mapper code is unchanged. Centralized Zod schemas in `packages/types` still validate read paths.
...
- Maintain the 15 partial unique indexes, 5 CHECK constraints, and 4 views as raw SQL: PG migrations on one side, `prisma/sqlserver/manual-ddl.sql` (applied after `db push`) on the other. The Prisma 7.8 `partialIndexes` preview is NOT adopted (the SQL Server `migrate` flow is blocked and the preview has open bugs). Composite filtered indexes with nullable members append `AND <col> IS NOT NULL` to reproduce PostgreSQL's NULL-distinct semantics.
- Force case-sensitive collation in SQL Server (`Latin1_General_100_CS_AS_SC_UTF8`) to keep uniqueness parity with PG (so `User.email`, `Magnitude.code`, etc. behave identically). Note: `ORDER BY` sort order still differs (documented parity gap).
- Provider-aware referential actions: the SQL Server schema sets `onUpdate/onDelete: NoAction` (SQL Server forbids multiple cascade paths / `Restrict`).
