...
- [x] 8b.9 Filter `findReferencedEmissionFactors` on `ACTIVE`, so a client holding pre-delete state cannot silently re-attach a line to a factor the catalogue no longer offers. **Landed in `add-emission-factor-year` instead**, which needed the same filter for its own reasons and carries it with a fuller rationale; this change was rebased onto it and its own version dropped as redundant. The behaviour the spec requires is unchanged.
