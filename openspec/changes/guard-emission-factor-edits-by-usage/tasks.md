...
- [x] 8b.9 Stop a client holding pre-delete state from silently re-attaching a line to a factor the catalogue no longer offers. **Landed in `add-emission-factor-year` instead.** This change first did it by filtering `findReferencedEmissionFactors` on `ACTIVE`; that branch needed the same guarantee, took it over, and has since gone further: the lookup reads the status rather than filtering on it, and the sync refuses such a payload with `INVALID_EMISSION_FACTOR_REFERENCE` (422, `details.reason: DELETED`) instead of saving the line without its factor. The refusal is now split by cause: an id matching no factor row at all is answered with `EmissionFactorNotFoundError` (404), leaving the 422 for a factor that exists but the footprint may not use. This change's own version was dropped on rebase as redundant, and the scenario in the spec now states the 422.
- [x] 8b.10 Detach the unclaimed lines on the **update** path too, not only on delete. The guard only proves no claimed footprint depends on the factor, so an edit that moves it out of the context its lines were offered it in — subcategory, year, source, rate unit, or a dimension value that narrows — strands them exactly as a delete would. `value` is excluded: freezing it is what the snapshot is for, and a dimension value that becomes null widens the factor's reach rather than narrowing it.
