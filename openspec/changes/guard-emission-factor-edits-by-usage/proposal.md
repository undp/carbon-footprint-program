...
- **Expose the usage on the maintainer listing.** `GetAllEmissionFactorsResponse` gains `referencedLineCount`, so the grid can disable the row and say _"Usado por N líneas"_ instead of offering an edit that will 409.
