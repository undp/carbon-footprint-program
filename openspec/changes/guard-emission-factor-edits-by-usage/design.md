...
Unclaimed anonymous footprints are excluded, and this was missed in the first cut. The calculator is open: `createCarbonInventory` is a public route that binds the new footprint to the **published** version, and `syncCarbonInventoryLines` is anonymous, so any visitor who picks a subcategory and types a quantity references a catalogue factor. Nothing can then remove that reference — `deleteCarbonInventory` is private, so the UUID holder is refused; its domain hook grants an org-less footprint only to `createdById`, which is null until the footprint is claimed; and no write route sets `canAdminsBypass`, so an administrator is refused too. Counting them handed anonymous traffic a permanent freeze on the live catalogue, which is the opposite of what this change is for — and the factor was unreplaceable as well, because `checkDuplicateEmissionFactor` and `validateSourceConsistency` reject a second factor for the same `(subcategory, year)`.

The exclusion is `NOT { organizationId: null, createdById: null }`, the complement of the pair `claimCarbonInventory` matches on, so a footprint starts blocking the moment it has an owner who can retire it. Their lines are still counted, separately, and reported as `unclaimedReferencedLineCount`: a delete is allowed and its confirmation names them. This is the warning-instead-of-block shape Decision 9 of `add-emission-factor-year` deferred, applied to the one cohort where a block could never be lifted by any actor.
