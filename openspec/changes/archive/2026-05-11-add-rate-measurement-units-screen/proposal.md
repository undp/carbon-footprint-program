...
- **API**: `apps/api/src/features/measurementUnits/getAllRateMeasurementUnits/` is updated:
  - `service.ts` runs the main `findMany` plus two `groupBy({ by: ["rateMeasurementUnitId"|"appliedFactorRateUnitId"], _count: { _all: true } })` queries in parallel to compute per-row counts. The number of queries is fixed at three (one main + two counts) regardless of result size.
