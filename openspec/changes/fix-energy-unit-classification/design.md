...
**Rationale**: `0.0036` and `3.6` round to representations with acceptable precision for this conversion in `double precision` (the underlying type of the `Float`-mapped `base_factor` column) — they are not mathematically exact in binary floating point, but the rounding error is many orders of magnitude smaller than the truncation we would incur with the repeating decimal `277.7777…` that the alternative requires. Conversion accuracy is preferred over preserving the current `kWh = 1.0` value, which is internal and not user-visible.
...
- **`baseFactor` precision** → `0.0036` and `3.6` are stored in `double precision` with acceptable rounding for this conversion (they are not mathematically exact in binary floating point), and conversions involving multiple multiplications may accumulate floating-point error. The existing emission-factor conversion logic at `helper.ts` already operates in `Number` and clamps to a fixed precision downstream; no new precision risk is introduced.
