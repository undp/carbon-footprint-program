-- Chatbot: conversations, messages and the retrieval corpus (sources, chunks
-- with their embeddings, ingest runs). Requires the pgvector extension on the
-- PostgreSQL server (see docs/operations/production-deployment.md).

-- Install pgvector first so the vector(1024) type is recognized.
...
-- CreateIndex: HNSW index using cosine distance with pgvector defaults
-- (m=16, ef_construction=64). Search-time ef_search is intentionally not
-- pinned (default 40).
