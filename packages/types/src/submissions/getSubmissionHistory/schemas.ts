...
export const GetSubmissionHistoryQuerySchema = z
  .object({
    carbonInventoryId: IdSchema.optional().describe(
      "The carbon inventory ID to get history for"
    ),
    organizationId: IdSchema.optional().describe(
      "The organization ID to get history for"
    ),
  })
  .refine(
    (data) =>
      (!!data.carbonInventoryId && !data.organizationId) ||
      (!data.carbonInventoryId && !!data.organizationId),
    {
      message: "Provide exactly one of carbonInventoryId or organizationId",
    }
  );
