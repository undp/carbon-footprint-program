...
Whatever is not in the seed lives in typed configuration files in the frontend and the API. The
changes are small, but they are compiled into the image: each country keeps its own branch or fork
with these values. We estimate 1–2 weeks, in parallel with [phase 2](./02-seed-content.md).
...
| Demo-environment notice on the landing page                    | [`apps/web/src/screens/Landing/components/LandingHero.tsx`](../../apps/web/src/screens/Landing/components/LandingHero.tsx)                    | Always shown today; remove or replace it in production                             |
...
| `APP_LOCALE`                                      | [`apps/web/src/config/constants.ts`](../../apps/web/src/config/constants.ts)             | `es-ES` (1.234,56)                  | The country's locale; `es-MX` or `es-DO` switch separators to 1,234.56               |
...
| `CALCULATOR_YEARS_RANGE_FROM_CURRENT`             | [`apps/web/src/config/constants.ts`](../../apps/web/src/config/constants.ts)             | 5                                   | Past years in which the maintainer can date a factor                                 |
...
| `MEASURING_ORGANIZATIONS_YEAR_RANGE`              | [`apps/api/src/config/constants.ts`](../../apps/api/src/config/constants.ts)             | 2                                   | "Measuring organizations" window; changing it also requires its view's SQL migration |
