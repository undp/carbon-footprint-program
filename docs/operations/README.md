...
| [Docker Compose — Full Stack](./docker-compose.md)               | End-to-end guide for `docker-compose.yml`: services overview, env configuration, auth + storage SP setup (portal + CLI), common workflows, prod notes, troubleshooting                     |
| [Production Deployment (on-premise)](./production-deployment.md) | Running `docker-compose.prod.yml` against an external PostgreSQL: env setup, DB roles & privileges, manual migrations/seed runbook, troubleshooting                                        |
| [RD Deployment Branch](./rd-deployment-branch.md)                | Why the Dominican deployment runs `rd/integration`, what the branch carries, the frozen-upstream policy (security cherry-picks only, no periodic rebase), and why reseeding is destructive |
