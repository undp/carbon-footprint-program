...
| `test_statement_coverage90`   | MUST  | ❌     | `apps/api` now enforces **90%** statement coverage in CI (the `coverage` job merges the test legs natively via `vitest run --merge-reports --coverage`); `apps/web` is now unit-tested across its logic layers behind an enforced floor but sits at only ~8% overall statement coverage (its render-heavy `screens/`/`components/` are untested), so the project-wide 90% bar is unmet. |
