...
- **CORS policy** (`@fastify/cors`) and **rate limiting** (`@fastify/rate-limit`, 100 requests/minute per IP) are applied to API routes. Azure Front Door provides WAF capabilities as an optional layer. **Security HTTP headers via `@fastify/helmet` are tracked as a Sprint 2 remediation item**: the dependency is declared but the plugin is not yet registered. See [`docs/security/hardening.md`](../security/hardening.md) and the matching entry in [`./acceptable-use.md`](./acceptable-use.md#current-controls).
...
- **File MIME-type validation and antivirus scanning** for uploaded documents are tracked as Sprint 2 remediation items. Currently the API verifies blob existence and reads the MIME type from Azure's storage metadata, but does not independently validate the content against the declared type or scan for malware.
- **HTTP security headers via `@fastify/helmet`** are tracked as a Sprint 2 remediation item. The dependency is installed but the plugin is not yet registered, so headers such as `X-Content-Type-Options`, `X-Frame-Options` and a strict `Content-Security-Policy` are not currently set by the API. See [`./acceptable-use.md`](./acceptable-use.md#current-controls) for the matching gap entry and the acceptance criteria.
