...
The default is `false` (trust nothing), which preserves the behaviour of deployments that upgrade without setting it. When `NODE_ENV=production` and the variable is unset, the API logs a warning at boot. On Azure the value comes from the `apiTrustProxy` parameter in `infra/main.bicep` rather than an env file; local development needs nothing, since the browser reaches the API directly.
