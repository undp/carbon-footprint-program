...
| `AZURE_OPENAI_REASONING_EFFORT`          | No                                     | unset                      | Optional reasoning-effort hint propagated to the chat `chat.completions.create` call. Allowed values: `minimal`, `low`, `medium`, `high`. Applies only to reasoning chat deployments (gpt-5 family, o-series); `minimal` is the recommended value for chat with streaming. Leave unset for non-reasoning models (gpt-4.1, gpt-4o) — the SDK may reject the parameter. |
