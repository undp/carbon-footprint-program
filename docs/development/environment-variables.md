...
**Hop counts (`1`, `2`, any bare integer other than `0`) are rejected and fail the boot.** They were the recommended Azure values until Fastify 5.12.1 removed the feature ([GHSA-3m5p-2c4r-xxw2](https://github.com/advisories/GHSA-3m5p-2c4r-xxw2)), because counting hops cannot validate the immediate peer. Fastify now treats a numeric `trustProxy` as trusting _nothing_ instead of erroring, so forwarding one would leave a deployment looking configured while the rate limiter quietly returned to a single shared bucket. On Azure App Service use `linklocal` instead; see [Hop counts are no longer supported](../security/hardening.md#hop-counts-are-no-longer-supported). `0` is the exception: trusting zero hops is trusting nothing, so it is accepted and folded to `false` rather than breaking a deployment whose behaviour would not change.
...
| Variable      | Required | Default                        | Description                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------- | -------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TRUST_PROXY` | No       | unset (acts as `false`, warns) | `false` = trust nothing, decision recorded. A comma-separated IP/CIDR list (`10.0.0.0/8,192.168.0.0/16`) = trust only those senders — **preferred**. `loopback` / `linklocal` / `uniquelocal` = Fastify's named ranges (`linklocal` for Azure App Service). `true` = trust the whole chain (last resort). `0` = accepted as `false`. Any other bare integer = **rejected, fails the boot**. |
...
| `CHATBOT_MAX_TURNS_PER_MINUTE`           | No                                               | `15`                       | Chatbot turns allowed per minute, per IP, on `POST /api/chatbot/message`. Tighter than the global 100 req/min because a turn invokes a model and the rest of the API does not. Per-deployment rather than a constant: a country whose users all egress through one address shares a single bucket and may need a higher ceiling, and the integration suite drives more turns per file than any real caller would. Must be an integer ≥ 1 — the API refuses to boot otherwise. To switch the assistant off, set `CHATBOT_ENABLED=false` rather than limiting it to zero. |
...
| `AZURE_OPENAI_DEPLOYMENT_NAME`           | When enabled + `azure-openai`                    | —                          | Azure OpenAI chat deployment name. Authenticates via `DefaultAzureCredential` (managed identity) by default, or via `AZURE_OPENAI_API_KEY` when that key is set.                                                                                                                                                                                                                                                                                                                                                                                                        |
| `AZURE_OPENAI_API_VERSION`               | No                                               | `2024-10-21`               | Azure OpenAI API version used by the chat client. Promoted to an env var so operators can bump it per deployment without code changes.                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `AZURE_OPENAI_API_KEY`                   | No (dev only)                                    | unset                      | Optional API key fallback for local development. Production SHALL leave this unset and rely on managed identity. When set, both the chat and embeddings clients use API key auth.                                                                                                                                                                                                                                                                                                                                                                                       |
| `AZURE_OPENAI_REASONING_EFFORT`          | No                                               | unset                      | Optional reasoning-effort hint passed to `chat.completions.create`. Allowed values: `minimal`, `low`, `medium`, `high`. Applies only to reasoning deployments (gpt-5 family, o-series); `minimal` is recommended for streaming chat. Leave unset for gpt-4.1 / gpt-4o — the SDK may reject it.                                                                                                                                                                                                                                                                          |
| `EMBEDDING_PROVIDER`                     | No                                               | `mock`                     | Selects the embeddings backend for the RAG corpus. Allowed values: `mock`, `azure-openai`. When the chatbot is enabled (`CHATBOT_ENABLED=true`), the API refuses to boot with `mock` in production — the mock returns SHA-256-derived vectors, so cosine similarity over them is random and would silently corrupt retrieval. The `chatbot:ingest` CLI refuses `mock` in production **regardless of `CHATBOT_ENABLED`**, because seeding a corpus before switching the assistant on is a supported workflow and ingest is what writes the vectors.                      |
| `AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME` | When enabled + `EMBEDDING_PROVIDER=azure-openai` | —                          | Azure OpenAI embeddings deployment name. Separate from the chat deployment because embeddings use their own model (`text-embedding-3-large`).                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `AZURE_OPENAI_EMBEDDING_API_VERSION`     | No                                               | `AZURE_OPENAI_API_VERSION` | Optional override for the embeddings API version. Defaults to the chat-client version, for deployments whose chat and embedding endpoints expose different SDK contracts.                                                                                                                                                                                                                                                                                                                                                                                               |
