...
- Step 3 saves **implicitly** on every navigation (category tab, **Volver**, **Siguiente**, exit) and also **explicitly**: next to the step title, the status reads _Cambios sin guardar_ / _Sin cambios pendientes_ / _Guardando cambios…_, and the save icon beside it is enabled only while there are changes to persist.
