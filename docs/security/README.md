...
| [Frontend Auth Recovery](./frontend-auth-recovery.md) | Recovery flow when MSAL login succeeds but `GET /users/me` fails: cleanup paths in `AuthContext` and `requireRole`, Landing snackbar handoff            |
