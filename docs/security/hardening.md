...
| 2. API at request-upload time        | `RequestUploadBodySchema` requires `originalName`, `fileType`, `sizeBytes` and `mimeType`. The service resolves effective limits via `getFileUploadLimits()` and rejects with `FILE_SIZE_OUT_OF_RANGE`, `FILE_MIME_TYPE_NOT_ALLOWED`, or `FILE_EXTENSION_NOT_ALLOWED` before signing a SAS. |
