...
| `AZURE_OPENAI_API_KEY` unset in Production        | ✅ Enforced when enabled  | Bicep never sets it, and the account deploys with `disableLocalAuth: true` — key auth is refused at the resource, so managed identity is the only path                    |
