...
This document covers how the web app recovers when **OIDC authentication succeeds but the follow-up `GET /users/me` request fails**, leaving the user in a half-authenticated state.
...
For the IdP tenant and app registration setup, see [Azure / OIDC auth setup](../infrastructure/AzureAuthenticationSetup.md).
...
1. **OIDC session** — the OAuth/OIDC session managed by `oidc-client-ts` (the `UserManager`). Created after a successful login against the configured IdP (Entra, Keycloak, …).
...
These two sessions are created by separate calls. The OIDC login can succeed without the app user being available — for example if the API is down, returns a 5xx, the user was revoked on the backend, the network blips mid-init, or a SAS/identity-related dependency fails.

---

## Previous behavior (the risk)

Before this change, when `/users/me` failed after a successful login:
...
The user was effectively stranded: authenticated to the IdP, unknown to the app, with no recovery path.
...
- **Security**: leaving an OIDC session active without an app session means the next page load retries with the same broken state, never re-prompting for login.

---

## Solution

A coordinated, two-path cleanup that detects the half-authenticated state, drops the local OIDC session **without an IdP round-trip**, evicts the failed React Query cache entry, clears app state, and redirects the user back to Landing with a Spanish error snackbar.
...
The `removeUser` call is wrapped in `try/catch`: a transient OIDC failure must not block the rest of the cleanup, otherwise the user would be stranded in a half-broken session.
...
1. Performs **best-effort cleanup**: `oidcUserManager.removeUser()` and `queryClient.removeQueries({ queryKey: userKeys.me })`. Both are wrapped in their own `try/catch` and any error is logged — cleanup failures must never block the redirect.
...
│                                                              │
│   User logs in → OIDC session created                        │
│        │                                                     │
│        ▼                                                     │
│   useInitializeUser → useMe → GET /users/me ❌ fails         │
│        │                                                     │
│        ▼                                                     │
│   AuthContext effect detects isUserError                     │
│        │                                                     │
│        ▼                                                     │
│   handleLoginFailure:                                        │
│     • oidc.removeUser()                                       │
...
│                                                              │
│   User opens /app/... directly with an existing OIDC session │
│        │                                                     │
│        ▼                                                     │
...
│        │                                                     │
│        ▼                                                     │
│   Best-effort cleanup (removeUser + removeQueries)           │
│        │                                                     │
│        ▼                                                     │
...
| Decision                                     | Reason                                                                                                                |
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `removeUser` over `signoutRedirect`          | Avoids the IdP round-trip so the in-memory snackbar survives the navigation                                           |
...
| Cleanup wrapped in `try/catch` in both paths | A flaky OIDC or React Query call must not block the redirect — leaving the user stranded is worse than a noisy log    |
...
| Direct navigation to a protected route with a stale OIDC session | `requireRole` guard path                                           |
...
| `removeUser` itself fails                                        | Wrapped in `try/catch`; cleanup continues, redirect still fires    |
