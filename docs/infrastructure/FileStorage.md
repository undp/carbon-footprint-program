...
- `MINIO_PUBLIC_BASE_URL` — opt-in storage relay (see [Storage relay](#storage-relay-keeping-minio-internal)). When set, presigned URLs are rewritten to this public base and the API proxies them, so MinIO stays internal.
...
By default the browser talks to MinIO directly via presigned URLs, which means MinIO must be reachable from the browser (a public HTTPS endpoint) and must allow the web origin via CORS. When that is not acceptable — e.g. MinIO sits on an internal network that must not be exposed — set `MINIO_PUBLIC_BASE_URL` to turn the API into a reverse-proxy ("relay") for storage:

- The presigned URL is still **signed against the internal `MINIO_ENDPOINT`**, then its origin is rewritten to `MINIO_PUBLIC_BASE_URL` (e.g. `https://api.example.cl/api/storage`). The signature (path + query) is preserved verbatim.
- The browser hits `<MINIO_PUBLIC_BASE_URL>/<bucket>/<key>?X-Amz-...` (an API route), and the API forwards the request **unchanged** to the internal endpoint. Because the forward preserves the signed host/path/query, MinIO revalidates the original signature — there is **no re-signing**.
...
`MINIO_PUBLIC_BASE_URL` must be the API's public origin plus the `/api/storage` path, and is MinIO-only (Azure serves SAS URLs directly over HTTPS). Leave it unset to keep the browser-direct behaviour described above. All file bytes flow through the API process, so size the API for the expected transfer volume.
...
| `MINIO_PUBLIC_BASE_URL`        | MinIO    | no          | —           | Relay base; rewrites presigned URLs (keeps MinIO internal) |
