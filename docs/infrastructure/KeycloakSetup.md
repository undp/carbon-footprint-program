...
3. **Schema param is `currentSchema`, not `schema`** — `?currentSchema=keycloak`. But prefer `KC_DB_SCHEMA` (above) over hand-writing it.
...
So the libpq-style `postgresql://keycloak:S3cr%24et@db.internal.example:5432/keycloak?schema=keycloak` becomes:

```bash
KC_DB_URL=jdbc:postgresql://db.internal.example:5432/keycloak?currentSchema=keycloak
