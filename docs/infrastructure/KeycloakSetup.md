...
The realm shape, the client, and the token contract are identical in both environments — only the operational surface differs. **Dev** runs Keycloak with a bundled, throwaway database, the relaxed `start-dev` command, and plain HTTP on `localhost:18080`. **Production** runs it with a durable database — bundled single-host or external managed — the hardened `start` command, and TLS terminated by a reverse proxy in front of a real public host. This single guide covers both, side by side.
...
| Database       | Throwaway Postgres in the same compose project           | A durable database — bundled single-host or external managed, mirroring how the app's own DB is deployed in [production-deployment.md](../operations/production-deployment.md)             |
...
All commands run from the **repo root**; Keycloak uses host **18080** to avoid the api's 8080, and `--project-directory .` keeps the realm-import paths rooted regardless of `-f` order. Details below.
...
**Why `localhost` is never valid here:** inside Keycloak, `localhost` is the Keycloak container itself; use a host/service name reachable from that container — the same rule as the [issuer/JWKS host split](#the-issuer-vs-jwks-host-split).
...
Prefer the component vars (`KC_DB_URL_HOST` / `KC_DB_URL_PORT` / `KC_DB_URL_DATABASE` + `KC_DB_SCHEMA`) — the overlay assembles a correct JDBC URL for you. Only set the full `KC_DB_URL` for things the components can't express (e.g. `?sslmode=require`). When you do, it must follow **pgJDBC** syntax, which is **not** the libpq / Prisma / SQLAlchemy `postgresql://user:pass@host/db` form. Getting this wrong yields `Failed to obtain JDBC connection` → `Driver does not support the provided URL`. Four rules:
...
> **Why the wildcard redirect?** The dev realm deliberately accepts any `http://localhost:*` redirect (and `*` web origins) so a dev server on any port completes the OIDC flow — this is what lets several git worktrees run at once, each on its own web port. It applies to this **dev-only** realm (`infra/keycloak/dev/realm-huella.dev.json`); the [production realm](#3-client-huella-web) whitelists exact URIs instead. See [Running several git worktrees](../development/worktree-isolation.md).
...
> **Re-verify on every image bump.** The `${HUELLA_WEB_ORIGIN}` substitution above is Keycloak's own env-substitution behavior at `--import-realm` time, and it can change between versions — whenever the Keycloak image pin (`compose/keycloak.prod.yaml`) is bumped, re-verify on a fresh import that `${HUELLA_WEB_ORIGIN}` still gets substituted into the realm's redirect URIs / web origins / post-logout URIs before relying on it in production.
...
| Offline session idle  | Only relevant if `offline_access` is used for long-lived refresh — the shipped realm uses 1 day (86400s; defense-in-depth, since the SPA no longer requests `offline_access` on Keycloak)                                                                                                                                                                                |
| Offline session max   | The shipped realm uses 2 days (172800s) — same defense-in-depth rationale as offline session idle                                                                                                                                                                                                                                                                        |
...
In proxy mode, bind HTTP to loopback or a firewalled proxy-only interface — direct access can spoof the forwarded headers (defeating brute-force accounting, among others). The env template therefore defaults to a loopback bind (`KC_HTTP_BIND=127.0.0.1:18080`); keep it when the proxy runs on the same host, and widen it to a firewalled proxy-only interface only when the proxy is remote. Additional hardening once mode A is working: restrict which hosts Keycloak trusts to send forwarded headers by setting [`KC_PROXY_TRUSTED_ADDRESSES`](https://www.keycloak.org/server/reverseproxy) in `.env.prod.keycloak` to the reverse proxy's own IP/CIDR — `compose/keycloak.prod.yaml` passes it through, and its default (`0.0.0.0/0,::/0`) mirrors Keycloak's own trust-all behavior when the option is unset. Leave `KC_HOSTNAME_STRICT` at its default (`true`) — it rejects requests with a mismatched `Host` header, which is what you want once the public hostname is fixed.
...
| `VITE_OIDC_SCOPES`                   | `openid profile email` (`access_as_user` is a default scope in Keycloak — not requested here)                                                                                                | `openid profile email` — baseline OIDC scopes. `access_as_user` is a **default** client scope (unlike Entra), so it must **not** be listed here. `offline_access` is intentionally **not** requested: silent renewal relies on the normal SSO-session-bound refresh token, so there's no need for a long-lived offline token in the browser (Azure/Entra still requires `offline_access` — see [Azure Entra Authentication Setup](./AzureAuthenticationSetup.md)). |
| `VITE_OIDC_REDIRECT_URI`             | `http://localhost:3000/auth/callback` (or your dev origin on any localhost port — the dev realm accepts `http://localhost:*`; defaults to `<origin>/auth/callback`)                          | `https://<web-host>/auth/callback` — must exactly match an entry in `huella-web`'s **Valid redirect URIs** ([Admin Console step 3](#3-client-huella-web)).                                                                                                                                                                                                                                                                                                         |
...
VITE_OIDC_SCOPES=openid profile email   # access_as_user is a default scope in Keycloak — not requested here; offline_access intentionally omitted (silent renew uses the SSO-session-bound refresh token)
VITE_OIDC_REDIRECT_URI=http://localhost:3000/auth/callback   # or your dev origin; defaults to <origin>/auth/callback
```

The dev realm's `huella-web` client accepts any `http://localhost:*` redirect URI and web origin, so the Docker web (`:3000`), a host Vite dev server (`:5173`), and any other localhost port (e.g. a parallel git worktree) all work out of the box.
...
| -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Keycloak exits at boot: `Failed to obtain JDBC connection` → `Driver does not support the provided URL`        | `KC_DB_URL` isn't a **JDBC** URL — a bare `postgresql://user:pass@host/db?schema=…` (libpq/Prisma form) instead of pgJDBC                                                                                                                                      | Use `jdbc:postgresql://host:port/db` (extras like `?sslmode=require` only, never `?currentSchema=` — set the schema via `KC_DB_SCHEMA`); move credentials to `KC_DB_USERNAME`/`KC_DB_PASSWORD` (raw, not `%`-encoded); escape `$` as `$$` — see [Overriding `KC_DB_URL`](#overriding-kc_db_url--it-must-be-a-jdbc-url). Better: leave `KC_DB_URL` empty and use the component vars + `KC_DB_SCHEMA`. |
...
| `redirect_uri` error at login                                                                                  | Web origin not in `huella-web`'s Valid redirect URIs, or `VITE_OIDC_REDIRECT_URI` doesn't match exactly ([generic explanation](../development/troubleshooting.md#oidc-redirect-uri-mismatch))                                                                  | Dev: any `http://localhost:<port>` is accepted; for a non-localhost host, add it to the client. Prod: add the exact `<web-host>` origin — [Admin Console step 3](#3-client-huella-web).                                                                                                                                                                                                              |
...
| A few connection-refused / JDBC-connection-failure log lines and container restarts right after the first `up` | Expected — `keycloak` has no `depends_on` on `keycloak-db` (by design, so the same compose file also works against an external managed Postgres), so Keycloak can start before the database is ready to accept connections                                     | Not a misconfiguration. `restart: unless-stopped` recovers it automatically once `keycloak-db` becomes reachable. Only actionable if the restart loop persists well beyond the DB's own startup time.                                                                                                                                                                                                |
