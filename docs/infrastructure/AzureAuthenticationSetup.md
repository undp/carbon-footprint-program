...
3. Configure:
   ...
   - **Redirect URIs** (Platform: Single-page application / SPA):
     ...
     - Running several git worktrees at once? SPA registrations don't allow wildcards, so register the dev port range too (`http://localhost:5174/auth/callback` … `5183/auth/callback`, plus the bare `http://localhost:5174` … origins for post-logout) — or use Keycloak for worktree dev. See [Running several git worktrees](../development/local-setup.md#running-several-git-worktrees-at-once-optional).
4. Click **"Register"**
