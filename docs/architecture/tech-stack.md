...
| **AI (chat)**           | openai (Azure OpenAI) + @azure/identity                       | 6.x     | Optional — powers the chatbot, gated by `CHATBOT_ENABLED`. Streaming chat completions with server-side tool calling; managed identity by default, API key as a documented dev-only fallback |
| **AI (embeddings)**     | openai (Azure OpenAI)                                         | 6.x     | Optional — `text-embedding-3-large` requested at 1024 dimensions to match the `vector(1024)` column. A deterministic mock backend keeps tests offline and is refused in production          |
| **PDF extraction**      | pdf-parse                                                     | 2.x     | Used only by the operator-run corpus ingest CLI to extract text from source PDFs — not reachable from any HTTP route                                                                        |
...
| **Extensions**   | pgvector           | 0.8.x (`vector`)            | Required only when `CHATBOT_ENABLED=true` — stores chunk embeddings as `vector(1024)` with an HNSW cosine index. Local and CI Postgres therefore run the `pgvector/pgvector:pg18` image; the official image cannot apply the migration. Modelled as `Unsupported("vector(1024)")` because Prisma has no native vector type, so retrieval uses raw SQL |
