...
## Nota sobre Solución Temporal

⚠️ **Importante**: Esta es una solución temporal hasta que se implemente una más robusta que:

- Se ejecute dentro de la red de Azure (sin necesidad de firewall público)
- Sea compatible con pipelines de CI/CD
- No requiera acceso desde IPs externas

La solución actual requiere:

- Configuración manual de IPs en el firewall
- Ejecución desde una máquina local con acceso a Azure CLI
- No es ideal para integración en pipelines automatizados

Se recomienda migrar a una solución que ejecute las migraciones dentro de la infraestructura de Azure, como:

- Un Azure Container Instance (ACI) con acceso a la red virtual
- Un job en Azure Container Apps
- Un paso en el pipeline de CI/CD que se ejecute dentro de la red de Azure
