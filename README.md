...
| Infrastructure as Code                    | Open / closed      | Azure Bicep (`infra/`) provisions the reference deployment; **cloud-neutral [Terraform examples](./infra/terraform/) for AWS and GCP** are provided for non-Azure adopters, and containers (`Dockerfile`, `docker-compose*.yml`) allow deployment to any container platform. |
