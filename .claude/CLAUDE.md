...
- **Decimal precision is `@db.Decimal(28, 10)` everywhere**: every `Decimal` column in `schema.prisma` MUST use `@db.Decimal(28, 10)`. Do not introduce new precisions or scales. Wider precision is preferred over loss when in doubt; the standard covers both high-precision emission factor calculations and human-readable scenario values.
- **UUIDs are generated client-side via `@default(uuid())`**: every UUID column in `schema.prisma` MUST use `@default(uuid())` (Prisma generates the UUID in JavaScript at insert time). Do not use `dbgenerated("gen_random_uuid()")` or any database-side UUID function — these are not portable across providers. Exception: a column may legitimately have no default when the caller supplies the UUID from an external system (e.g., `File.uuid` matches a blob-storage identifier); document the rationale in a comment.
