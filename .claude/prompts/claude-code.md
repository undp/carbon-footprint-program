...
6. Comment "@coderabbitai review" on the PR to trigger an incremental review.
7. Poll the PR comments every 60 seconds until CodeRabbit's review summary comment appears (this signals the review is complete). If no review summary appears after 1 hour, proceed without CodeRabbit review and note this in a PR comment.
8. Read all review comments from this review cycle (unresolved and without a fix commit). For each:
...
9. After addressing all comments, do a final pass: re-read the spec, run tests, confirm everything is consistent. If tests fail, fix the failures, run format/lint/type-check, commit, and push before proceeding.
10. Poll the PR for new review comments every 3 minutes. Process any new comments using the same rules from step 8, then repeat the final pass from step 9. After 30 minutes with no new comments, reduce polling frequency to every 30 minutes. Stop polling when the PR is approved.
