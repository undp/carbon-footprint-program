...
import type { ApproveReductionProjectResponse } from "@repo/types";
...
describe("POST /api/reduction-projects/:id/approve - Integration Tests", () => {
  ...
  describe("Happy path", () => {
    it("should approve a project in IN_REVIEW status", async () => {
      ...
      const response = await app.inject({
        method: "POST",
        url: `/api/reduction-projects/${project.id}/approve`,
      });

      expect(response.statusCode).toBe(200);
      const body = JSON.parse(response.body) as ApproveReductionProjectResponse;
      expect(body.status).toBe("APPROVED");
    });

    it("should mark the pending submission as APPROVED", async () => {
      const project = await createSubmittableReductionProject(
        prisma,
        organizationId,
        subcategoryId
  ...
  describe("Error cases", () => {
    ...
    it("should return 404 when project does not exist", async () => {
      const response = await app.inject({
        method: "POST",
        url: "/api/reduction-projects/99999999/approve",
      });
