...
import { SubCategoryStatus, CategoryStatus } from "@repo/types";
...
describe("POST /api/subcategories/ - Integration Tests", () => {
  ...
  describe("Successful creation", () => {
    it("should create a subcategory and return 201", async () => {
      ...
      expect(body.categoryId).toBe(category.id.toString());
      expect(body.status).toBe(SubCategoryStatus.ACTIVE);
    ...
    it("should persist the subcategory in the database", async () => {
      ...
      expect(dbRecord!.status).toBe(SubCategoryStatus.ACTIVE);
    });
