...
describe("activate CLI — integration", () => {
  ...
  it("flips OUTDATED on previous + ACTIVE on new in single transaction with shared timestamp", async () => {
    ...
    expect(v05!.deactivatedAt!.getTime()).toBe(v06!.activatedAt!.getTime());
  }, 60_000);
  ...
  it("advisory lock serializes concurrent activates with same (name, scope)", async () => {
    const NAME = "GHG Protocol Test";
  ...
  it("refuses non-DRAFT target with explicit Spanish error", async () => {
    const NAME = "GHG Protocol Test";
    ...
    // fires.
    expect(after!.activatedAt?.getTime()).toBe(outdated.activatedAt?.getTime());
