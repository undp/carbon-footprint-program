...
describe("ingest CLI — integration", () => {
  ...
  it("happy path with valid PDF", async () => {
  ...
  // untouched.
  it("re-ingest with new version creates new DRAFT alongside existing ACTIVE", async () => {
  ...
  // is created).
  it("re-ingest collision on existing DRAFT fails fast with non-zero exit and Spanish error", async () => {
    const NAME = "GHG Protocol Test";
    const seeded = await prisma.chatbotCorpusSource.create({
      data: {
        name: NAME,
  ...
  it("audit row persists on embedding failure with NULL completed_at and NULL source_id", async () => {
    const NAME = "GHG Protocol Test";
    const command = [
      "pnpm",
      "--filter=api",
