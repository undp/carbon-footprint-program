...
describe("Submission files — Full upload flow: request-upload → upload → confirm-upload → get files", () => {
  ...
  it("should complete the full lifecycle for an ATTACHMENT file", async () => {
    ...
    ) as RequestSubmissionUploadResponse;
    expect(uuid).toMatch(
    ...
    ) as ConfirmSubmissionUploadResponse;
  ...
  it("should allow multiple files to be uploaded on the same submission", async () => {
    ...
    expect(listResponse.statusCode).toBe(200);
    expect(
      JSON.parse(listResponse.body) as GetSubmissionFilesResponse
    ).toHaveLength(2);
  });
});
