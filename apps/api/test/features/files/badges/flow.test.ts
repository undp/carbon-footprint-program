...
describe(
  "Badge files — Full upload flow: request-upload → upload → confirm-upload → get files",
  () => {
    ...
    it("should complete the full lifecycle for a CARBON_INVENTORY badge", async () => {
      ...
      const { uuid, uploadUrl, expiresAt } = JSON.parse(requestResponse.body) as RequestBadgeUploadResponse;
      expect(uuid).toMatch(
      ...
      const confirmedFile = JSON.parse(confirmResponse.body) as ConfirmBadgeUploadResponse;
    ...
    it("should complete the full lifecycle for an ORGANIZATION_DATA badge", async () => {
      ...
      expect(listResponse.statusCode).toBe(200);
      expect(JSON.parse(listResponse.body) as GetBadgeFilesResponse).toHaveLength(1);
    });
