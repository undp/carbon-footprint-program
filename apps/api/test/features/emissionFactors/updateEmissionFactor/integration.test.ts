...
describe("PATCH /api/emission-factors/:id - Integration Tests", () => {
  ...
  describe("Concurrency", () => {
    ...
    // branch in the service's catch block (rather than relying on a
    // genuine, non-deterministic race), a separate transaction manually
    // holds an uncommitted row with the exact target uniqueness key open
    // while the service's own update runs: its pre-check passes (the
    // blocking row isn't visible yet), so it proceeds to UPDATE, which then
    // blocks on the DB's real partial unique index (both dimension value
    // columns must be non-null for the index to actually reject a
    // duplicate -- Postgres treats NULL <> NULL under standard
    // unique-index semantics). Once the blocking transaction commits, the
    // service's blocked UPDATE resumes and fails with a genuine
    // unique-constraint violation.
    it("should return 409 when a concurrent transaction commits a conflicting row while the update is in flight", async () => {
      const { subcategory, rateUnitId } = await buildBaseSubcategory(
      ...
      const initialDim1Value = await createTestEmissionFactorDimensionValue(
        prisma,
        dim1.id,
        { value: "Transactional Race Dim1 Initial" }
      );
      const targetDim1Value = await createTestEmissionFactorDimensionValue(
        prisma,
        dim1.id,
        { value: "Transactional Race Dim1 Target" }
      );
      ...
      const targetDim2Value = await createTestEmissionFactorDimensionValue(
        prisma,
        dim2.id,
        { value: "Transactional Race Dim2 Target" }
      );

      const victim = await createTestEmissionFactor(
        prisma,
        subcategory.id,
        rateUnitId,
        {
          source: "Test Transactional Race Source",
          dimensionValue1Id: initialDim1Value.id,
      ...
      const servicePromise = updateEmissionFactorService(
        prisma,
        victim.id.toString(),
        {
          dimensionValue1Name: "Transactional Race Dim1 Target",
          dimensionValue2Name: "Transactional Race Dim2 Target",
        },
        mapUserToResponse(dbUser)
      );

      // Give the service's own transaction enough time to pass its
      // pre-check and reach (and block on) the real UPDATE before we
  ...
  describe("Direct service invocation (bypassing schema-level validation)", () => {
    ...
    it("should return USER_NOT_FOUND when called without a user", async () => {
      const { subcategory, rateUnitId } = await buildBaseSubcategory(
        "Direct No User Update"
      );
      const ef = await createTestEmissionFactor(
        prisma,
        subcategory.id,
        rateUnitId,
        { source: "Test Direct No User Source" }
      );

      await expect(
        updateEmissionFactorService(
          prisma,
          ef.id.toString(),
          { value: 2.0 },
          null
