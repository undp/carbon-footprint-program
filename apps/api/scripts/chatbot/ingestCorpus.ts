...
const main = async (argv: string[]): Promise<number> => {
  ...
  // The config-level mock-in-production guard is gated on CHATBOT_ENABLED so a
  // deployment can pre-seed a corpus BEFORE turning the chatbot on. That leaves
  // this CLI as the last line of defence: it writes embeddings, and the mock
  // provider returns SHA-256-derived vectors with no semantic relation to the
  // text, so ingesting with it would silently poison retrieval. Refuse here,
  // independent of CHATBOT_ENABLED.
  if (IS_PROD && EMBEDDING_PROVIDER === "mock") {
    process.stderr.write(
      'EMBEDDING_PROVIDER="mock" no puede usarse para ingerir corpus cuando ' +
        "NODE_ENV=production: los vectores del mock no tienen relación semántica " +
        "con el texto y corromperían la búsqueda de forma silenciosa. " +
        'Configura EMBEDDING_PROVIDER="azure-openai" antes de ingerir.\n'
    );
    return 2;
  }
